...
class MiniModel:
    ...
    def cost_function(self, x, a=20, enable=True):
        c = np.exp(x*a)/a
        if enable:
            return np.round(c, 4)
        else:
...
def run_simulation():
    ...
    dfs = []
    for p in params:
        rs = simulation(epoch=100, time=10, param_set={'difficulty':p[0], 'payoff':p[1]})
...
#df = run_simulation()
#df.to_csv("/home/chery/Desktop/ACTR-Simon/data/mini_model_df_20220430.txt", index=False, header=False, mode="a")
